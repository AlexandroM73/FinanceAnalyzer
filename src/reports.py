...
def spending_by_category(transactions: pd.DataFrame, category: str, date: Optional[str] = None) -> pd.DataFrame:
    ...
    filtered_df = df[(df[date_col] >= three_months_ago) & (df[date_col] <= start_date) & (df["Категория"] == category)]

    # Группируем по месяцам и суммируем траты
    monthly_spending = (
        filtered_df.groupby(filtered_df[date_col].dt.to_period("M"))["Сумма операции"].sum().round(2).reset_index()
    )
...
def spending_by_workday(transactions: pd.DataFrame, date: Optional[str] = None) -> pd.DataFrame:
    ...
    if "Рабочий день" not in result["Тип дня"].values:
        result = pd.concat(
            [result, pd.DataFrame({"Тип дня": ["Рабочий день"], "Средняя сумма": [0.0]})], ignore_index=True
        )
    if "Выходной день" not in result["Тип дня"].values:
        result = pd.concat(
            [result, pd.DataFrame({"Тип дня": ["Выходной день"], "Средняя сумма": [0.0]})], ignore_index=True
        )
